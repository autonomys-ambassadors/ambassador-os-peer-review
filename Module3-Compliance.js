...
function runComplianceAudit() {
  ...
  try {
    ...
    Logger.log(`Error publishing anonymous scores to Google Sheets: ${error}`);
  }
  ...
  try {
    ...
    Logger.log(`Error publishing anonymous scores to Coda: ${error}`);
  }
}
...
// ===== CODA PUBLISHING FUNCTIONS =====

/**
 * Gets the Coda API key from Script Properties.
 * @returns {string|null} The Coda API key or null if not configured
 */
function getCodaApiKey() {
  const apiKey = PropertiesService.getScriptProperties().getProperty('CODA_API_KEY');
  if (!apiKey) {
    Logger.log('Coda API key not found in Script Properties. Set it in Project Settings > Script Properties.');
  }
  return apiKey;
}

/**
 * Makes an authenticated request to the Coda API.
 * @param {string} endpoint - API endpoint (e.g., '/docs' or '/docs/abc123/tables')
 * @param {string} method - HTTP method (GET, POST, PUT, DELETE)
 * @param {Object} payload - Optional request payload for POST/PUT
 * @returns {Object} Parsed JSON response
 */
function makeCodaApiRequest(endpoint, method, payload = null) {
  const apiKey = getCodaApiKey();
  if (!apiKey) {
    throw new Error('Coda API key not configured');
  }
  ...
  Logger.log(`Making Coda API request: ${method} ${url}`);

  const response = UrlFetchApp.fetch(url, options);
  const responseCode = response.getResponseCode();
  const responseText = response.getContentText();

  Logger.log(`Coda API response code: ${responseCode}`);

  if (responseCode < 200 || responseCode >= 300) {
    Logger.log(`Coda API error response: ${responseText}`);
    throw new Error(`Coda API request failed with status ${responseCode}: ${responseText}`);
  }
 ...
 * Creates a new doc from template and populates the table with score data.
 */
function publishAnonymousScoresToCoda() {
  // Check if Coda is configured
  if (!CODA_TEMPLATE_DOC_ID) {
    Logger.log('Coda template doc ID not configured. Skipping Coda score publishing.');
    return;
  }

  const apiKey = getCodaApiKey();
  if (!apiKey) {
    Logger.log('Coda API key not configured. Skipping Coda score publishing.');
    return;
  }
  ...
  try {
    ...
    // Collect anonymous score data (reuse existing function)
    const anonymousScores = collectAnonymousScoreData(monthName);
    ...
    const docTitle = TESTING
      ? `TEST - Ambassador Scores - ${monthName}`
      : `Ambassador Scores - ${monthName}`;

    // Copy template doc
    Logger.log(`Creating new Coda doc from template: ${docTitle}`);
    const copyPayload = {
      title: docTitle,
      sourceDoc: CODA_TEMPLATE_DOC_ID,
    };

    // Add folder if configured
    if (CODA_FOLDER_ID) {
      copyPayload.folderId = CODA_FOLDER_ID;
    }

    const copyResponse = makeCodaApiRequest('/docs', 'POST', copyPayload);
    const newDocId = copyResponse.id;

    Logger.log(`Successfully created Coda doc: ${newDocId}`);
    Logger.log(`Doc URL: ${copyResponse.browserLink || `https://coda.io/d/${newDocId}`}`);

    // Wait a bit for the doc to be ready
    Utilities.sleep(2000);

    // Get tables in the new doc
    const tablesResponse = makeCodaApiRequest(`/docs/${newDocId}/tables`, 'GET');
    if (!tablesResponse.items || tablesResponse.items.length === 0) {
      throw new Error('No tables found in the copied Coda doc. Ensure the template has a table.');
    }

    // Use the first table
    const tableId = tablesResponse.items[0].id;
    Logger.log(`Using table: ${tableId} (${tablesResponse.items[0].name})`);

    // Populate the table with score data
    populateCodaTable(newDocId, tableId, anonymousScores);

    Logger.log(
      `Successfully published ${anonymousScores.length} anonymous scores to Coda doc ${newDocId}`
    );
  } catch (error) {
    Logger.log(`Error in publishAnonymousScoresToCoda: ${error}`);
    // Don't throw - allow the Google Sheets publishing to continue
  }
}

/**
 * Populates a Coda table with anonymous score data.
 * @param {string} docId - The Coda doc ID
 * @param {string} tableId - The table ID within the doc
 * @param {Array} anonymousScores - Array of score objects to insert
 */
function populateCodaTable(docId, tableId, anonymousScores) {
  Logger.log(`Populating Coda table ${tableId} with ${anonymousScores.length} rows`);

  // Get the columns from the table to map field names
  const columnsResponse = makeCodaApiRequest(`/docs/${docId}/tables/${tableId}/columns`, 'GET');
  const columns = columnsResponse.items;

  Logger.log(`Table has ${columns.length} columns: ${columns.map((c) => c.name).join(', ')}`);

  // Build rows payload
  // Coda expects rows in format: { cells: [{ column: 'columnId', value: 'value' }, ...] }
  const rows = anonymousScores.map((score) => {
    const cells = [];

    // Map our score fields to Coda columns
    const fieldMapping = {
      Submitter: score.Submitter,
      'Primary Team': score['Primary Team'],
      Contributions: score.Contributions,
      'Score-1': score['Score-1'],
      'Remarks-1': score['Remarks-1'],
      'Score-2': score['Score-2'],
      'Remarks-2': score['Remarks-2'],
      'Score-3': score['Score-3'],
      'Remarks-3': score['Remarks-3'],
      'Final Score': score['Final Score'],
    };

    // Match columns by name and add cells
  ...
  // Insert rows in batches (Coda API has limits)
  const batchSize = 100;
  for (let i = 0; i < rows.length; i += batchSize) {
    const batch = rows.slice(i, i + batchSize);
    Logger.log(`Inserting batch ${Math.floor(i / batchSize) + 1}: rows ${i + 1} to ${i + batch.length}`);

    const payload = {
      rows: batch,
    };

    makeCodaApiRequest(`/docs/${docId}/tables/${tableId}/rows`, 'POST', payload);
    Logger.log(`Successfully inserted ${batch.length} rows`);
  ...
  Logger.log(`Finished populating Coda table with ${anonymousScores.length} rows`);
}
