...
function selectCRTMembers() {
	const registrySheet =
		SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Registry");
	const crtSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
	...
	const eligibleAmbassadors = registryData.filter(
		(email) =>
			email &&
			!email.toUpperCase().startsWith("(EXPELLED)") &&
			!recentCRTMembers.includes(email)
	);
	...
	if (eligibleAmbassadors.length < 5) {
		...
		if (!testing) {
			SpreadsheetApp.getUi().alert(
				"Failed to select CRT: not enough eligible ambassadors."
			);
		}
		return;
	}
