...
const AMBASSADORS_SCORES_DISCORD_HANDLES_COLUMN =
	"Ambassadors' Discord Handles";
...
const EVALUATIONS_COLUMN1 =
	"Score or Evaluator's Email (if left unevaluated)-1"; // evaluations as they come
const EVALUATIONS_COLUMN2 =
	"Score or Evaluator's Email (if left unevaluated)-2"; // evaluations as they come
const EVALUATIONS_COLUMN3 =
	"Score or Evaluator's Email (if left unevaluated)-3"; // evaluations as they come
...
function requestSubmissions() {
	...
	const scoresSpreadsheet = SpreadsheetApp.openById(
		AMBASSADORS_SCORES_SPREADSHEET_ID
	); // Open the "Ambassadors' Scores" spreadsheet
	...
	const registrySheet = SpreadsheetApp.openById(
		AMBASSADOR_REGISTRY_SPREADSHEET_ID
	).getSheetByName(REGISTRY_SHEET_NAME); // Open the "Registry" sheet
	...
	const month = Utilities.formatDate(
		...
		spreadsheetTimeZone,
		"MMMM"
	); // Format the deliverable date to get the month name
	const year = Utilities.formatDate(
		deliverableDate,
		spreadsheetTimeZone,
		"yyyy"
	); // Format the deliverable date to get the year
	...
	emails.forEach((email, index) => {
		...
		const message = REQUEST_SUBMISSION_EMAIL_TEMPLATE.replace(
			"{AmbassadorName}",
			discordHandle
		)
			.replace("{Month}", month)
	...
	PropertiesService.getScriptProperties().setProperty(
		...
		submissionWindowStart.toISOString()
	);
...
function setupSubmissionReminderTrigger(submissionStartTime) {
	...
	triggerDate.setMinutes(
		triggerDate.getMinutes() +
			...
			SUBMISSION_WINDOW_DAYS_SPACE
	); // Set trigger for 6 days after start for reminder
...
function checkNonRespondents() {
	...
	const submissionWindowStartStr =
		PropertiesService.getScriptProperties().getProperty(
			"submissionWindowStart"
		);
	if (!submissionWindowStartStr) {
		Logger.log(
			"Submission window start time not found, aborting checkNonRespondents."
		);
		return;
	}
	const submissionWindowStart = new Date(submissionWindowStartStr);
	const submissionWindowEnd = new Date(submissionWindowStart);
	submissionWindowEnd.setMinutes(
		submissionWindowStart.getMinutes() + SUBMISSION_WINDOW_DAYS
	);
	Logger.log(
		`Submission window is from ${submissionWindowStart} to ${submissionWindowEnd}`
	);
	...
	const responseData = formResponseSheet
		.getRange(
			2,
			1,
			formResponseSheet.getLastRow() - 1,
			formResponseSheet.getLastColumn()
		)
		.getValues();
	...
	const validResponses = responseData.filter((row) => {
		...
		return (
			timestamp >= submissionWindowStart && timestamp <= submissionWindowEnd
		);
	});
	Logger.log(
		`Valid responses within submission window: ${validResponses.length}`
	);
	...
	const nonRespondents = allEmails.filter(
...
function sendReminderEmails(nonRespondents) {
	...
	nonRespondents.forEach((email) => {
		...
		if (result) {
			...
			const message = REMINDER_EMAIL_TEMPLATE.replace(
				"{AmbassadorName}",
				discordHandle
			);
			...
			if (!testing) {
				...
				Logger.log(
					`Reminder email sent to: ${email} (Discord: ${discordHandle})`
				);
			} else {
...
function collectSubmissions() {
	...
	if (!submissionWindowStart) {
		Logger.log(
			"Submission window start not found, aborting submission collection."
		);
		return;
	}
	const submissionWindowEnd = new Date(submissionWindowStart);
	...
	const formResponseSheet = SpreadsheetApp.openById(
		AMBASSADOR_REGISTRY_SPREADSHEET_ID
...
function requestEvaluationsBranch() {
	...
	PropertiesService.getScriptProperties().setProperty(
		"evaluationStartTime",
		evaluationStartTime.toISOString()
	);
...
function createMonthSheetAndOverallColumn() {
	try {
		...
		const overallScoreSheet = scoresSpreadsheet.getSheetByName(
			OVERALL_SCORE_SHEET_NAME
		);

		if (!overallScoreSheet) {
			Logger.log(
				`Sheet "${OVERALL_SCORE_SHEET_NAME}" isn't found in "Ambassadors' Scores" sheet.`
			);
			return;
		}
		...
		Logger.log(
			`Previous month date: ${deliverableMonthDate} (ISO: ${deliverableMonthDate.toISOString()})`
		);

		// Form the name of the month, e.g. 'September 2024'
		const deliverableMonthName = Utilities.formatDate(
			deliverableMonthDate,
			spreadsheetTimeZone,
			"MMMM yyyy"
		);
		...
		if (!monthSheet) {
			...
			monthSheet = scoresSpreadsheet.insertSheet(
				deliverableMonthName,
				sheetIndex
			);
		...
		const existingColumns = overallScoreSheet
			...
			.getValues()[0];
		Logger.log(
			`Existing columns in "Overall Scores": ${existingColumns.join(", ")}`
		);

		// Check if there is a column for the previous month
		const columnExists = doesColumnExist(
			existingColumns,
			deliverableMonthDate,
			spreadsheetTimeZone
		);
		if (columnExists) {
			Logger.log(
				`Column for "${deliverableMonthName}" already exists in "Overall Scores". Skipping creation.`
			);
			return;
		}
		...
		Logger.log(
			`Column for "${deliverableMonthName}" succefully added to "Overall Scores".`
		);
	} catch (error) {
...
function findInsertIndexForMonthSheet(scoresSpreadsheet) {
	...
	sheets.forEach((sheet, index) => {
		...
		if (isMonthSheet(sheetName)) {
			...
			Logger.log(
				`Found a month sheet on the index ${index + 1}: "${sheetName}"`
			);
		}
	});
...
function getPreviousMonthDate(timeZone) {
	...
	const targetDate = new Date(
		Date.UTC(prevYear, prevMonth - 1, 1, 14, 0, 0, 0)
	);
	Logger.log(
		`Calculated date of the previous month: ${targetDate} (ISO: ${targetDate.toISOString()})`
	);
...
function doesColumnExist(existingColumns, targetDate, timeZone) {
	const targetMonthName = Utilities.formatDate(
		targetDate,
		timeZone,
		"MMMM yyyy"
	);
	...
	return existingColumns.some((column) => {
		if (column instanceof Date) {
			const columnMonthName = Utilities.formatDate(
				column,
				timeZone,
				"MMMM yyyy"
			);
...
function findInsertIndexForMonth(existingColumns) {
	...
	existingColumns.forEach((columnName, index) => {
		if (isMonthColumn(columnName)) {
			...
			Logger.log(
				`Month-column found on the index ${index + 1}: "${columnName}"`
			);
		}
	});
...
function isMonthColumn(columnValue) {
	...
	Logger.log(
		`Checking if "${columnValue}" is a month-column: isFirstDay=${isFirstDay}`
	);
...
function generateReviewMatrix() {
	...
	const registrySpreadsheet = SpreadsheetApp.openById(
		AMBASSADOR_REGISTRY_SPREADSHEET_ID
	);
	const registrySheet = registrySpreadsheet.getSheetByName(REGISTRY_SHEET_NAME);
	const formResponseSheet = registrySpreadsheet.getSheetByName(
		FORM_RESPONSES_SHEET_NAME
	);
	const reviewLogSheet = registrySpreadsheet.getSheetByName(
	...
	if (!submissionWindowStartStr) {
		Logger.log(
			"Submission window start time not found. Exiting generateReviewMatrix."
		);
		return;
	}
	const submissionWindowStart = new Date(submissionWindowStartStr);
	...
	const responseData = formResponseSheet
		.getRange(2, 1, lastRow - 1, formResponseSheet.getLastColumn())
		.getValues();
	...
	Logger.log(
		`Found ${validResponses.length} valid submissions within the submission window.`
	);
	...
	submittersEmails.forEach((submitter) => {
		...
		const availableEvaluators = evaluatorQueue.filter(
			(email) => email !== submitter
		);
		Logger.log(
			`Available evaluators for ${submitter}: ${JSON.stringify(
				availableEvaluators
			)}`
		);
	...
	assignments.forEach((assignment, index) => {
		...
		assignment.reviewers.forEach((reviewer, idx) => {
			reviewLogSheet
				.getRange(index + 2, idx + 2)
				.setValue(reviewer || "Has No Evaluator");
		});
	});
...
function sendEvaluationRequests() {
	try {
		...
		const reviewLogSheet = SpreadsheetApp.openById(
			AMBASSADOR_REGISTRY_SPREADSHEET_ID
		).getSheetByName(REVIEW_LOG_SHEET_NAME);
		Logger.log(`Opened sheet: ${REVIEW_LOG_SHEET_NAME}`);

		// Getting spreadsheet Time Zone
		const scoresSheet = SpreadsheetApp.openById(
			AMBASSADOR_REGISTRY_SPREADSHEET_ID
		);
		...
		Logger.log(
			`Review Log Sheet - Last Row: ${lastRow}, Last Column: ${lastColumn}`
		);

		// Проверяем, есть ли данные для обработки
		if (lastRow < 2) {
			Logger.log(
				"No data in Review Log sheet. Exiting sendEvaluationRequests."
			);
			return;
		}

		// Get data from the sheet (starting from the second row, first through fourth columns)
		const reviewData = reviewLogSheet
			.getRange(2, 1, lastRow - 1, 4)
			.getValues(); // Evaluations matrix
		...
		reviewData.forEach((row, rowIndex) => {
			...
			const contributionDetails = getContributionDetailsByEmail(
				submitterEmail,
				spreadsheetTimeZone
			);
			...
			reviewersEmails.forEach((reviewerEmail) => {
				try {
					const evaluatorDiscordHandle =
						getDiscordHandleFromEmail(reviewerEmail);
					Logger.log(`Discord Evaluator: ${evaluatorDiscordHandle}`);

					// Forming a message for evaluation
					const message = REQUEST_EVALUATION_EMAIL_TEMPLATE.replace(
						"{AmbassadorName}",
						evaluatorDiscordHandle
					)
					...
					if (!testing) {
						...
						Logger.log(
							`Test mode: The evaluation request must be sent to ${reviewerEmail}`
						);
					}
				} catch (error) {
					Logger.log(
						`Error sending evaluation request to ${reviewerEmail}: ${error}`
					);
				}
			});
		});
	...
	try {
		...
		reviewData.forEach((row, index) => {
			...
			if (!submitterDiscordHandle) {
				Logger.log(
					`Discord handle not found for submitter email: ${submitterEmail}`
				);
				return;
			}

			Logger.log(
				`Row ${index + 2}: Submitter Discord Handle: ${submitterDiscordHandle}`
			);
			...
			const evaluatorsEmails = [row[1], row[2], row[3]].filter(
				(email) => email
			);
...
function processEvaluationResponse(e) {
	try {
		...
		if (!e || !e.response) {
			Logger.log(
				"Error: Event parameter is missing or does not have a response."
			);
			return;
		}
		...
		itemResponses.forEach((itemResponse) => {
			...
			Logger.log(
				`Question: ${question}, Answer: ${answer}, Type of answer: ${typeof answer}`
			);
		...
		Logger.log(
			`Submitter Discord Handle (provided): ${submitterDiscordHandle}`
		);
		...
		evaluationEndTime.setUTCMinutes(
			evaluationStartTime.getMinutes() + EVALUATION_WINDOW_DAYS
		);
		...
		const expectedSubmitters =
			getExpectedSubmittersForEvaluator(evaluatorEmail);
		if (!expectedSubmitters || expectedSubmitters.length === 0) {
			Logger.log(
				`No expected submitters found for evaluator: ${evaluatorEmail}`
			);
			return;
		}
		Logger.log(
			`Expected submitters for evaluator ${evaluatorEmail}: ${expectedSubmitters.join(
				", "
			)}`
		);

		// Use bruteforceDiscordHandle to find the best match
		const correctedDiscordHandle = bruteforceDiscordHandle(
		...
		if (!correctedDiscordHandle) {
			Logger.log(
				`Could not match Discord handle: ${submitterDiscordHandle} for evaluator: ${evaluatorEmail}`
			);
			return;
		}
		...
		if (!evaluatorDiscordHandle) {
			Logger.log(
				`Discord handle not found for evaluator email: ${evaluatorEmail}`
			);
			return;
		}
		...
		if (!submitterCell) {
			Logger.log(
				`Submitter ${submitterDiscordHandle} not found in month sheet.`
			);
			return;
		}
		...
		for (let col = 2; col <= 4; col++) {
			// Evaluator columns are 2, 3, and 4
			...
			if (cellValue === evaluatorDiscordHandle) {
				...
				Logger.log(
					`Updated grade for submitter ${submitterDiscordHandle} by evaluator ${evaluatorDiscordHandle}. Grade: ${grade}`
				);
				gradeUpdated = true;
				break;
			}
		...
		if (validGrades.length > 0) {
			...
			Logger.log(
				`Final score updated for submitter ${submitterDiscordHandle}: ${finalScore}`
			);
		}
	} catch (error) {
...
function updateOverallScores() {
	...
	if (!monthSheet) {
		Logger.log(
			`Month sheet "${monthSheetName}" not found. Exiting updateOverallScores.`
		);
		return;
	}
	...
	const headerValues = overallScoreSheet
		.getRange(1, 1, 1, lastColumn)
		.getValues()[0];
	...
	for (let col = 1; col <= headerValues.length; col++) {
		...
		if (cellValue instanceof Date) {
			const cellMonthName = Utilities.formatDate(
				cellValue,
				spreadsheetTimeZone,
				"MMMM yyyy"
			);
			...
			if (cellMonthName === monthSheetName) {
				...
				Logger.log(
					`Found month column "${monthSheetName}" at index ${monthColumnIndex}.`
				);
				break;
			}
		}
	...
	if (!monthColumnIndex) {
		Logger.log(
			`Month column "${monthSheetName}" not found in "Overall Scores" sheet. Exiting updateOverallScores.`
		);
		return;
	}

	// Create a mapping of Discord handles to row numbers in the "Overall Scores" sheet
	const overallScoreData = overallScoreSheet
		.getRange(2, 1, overallScoreSheet.getLastRow() - 1, 1)
		.getValues(); // Only Discord handles
	...
	for (let i = 0; i < overallScoreData.length; i++) {
		...
		if (discordHandle) {
			...
			Logger.log(
				`Mapped Discord handle "${discordHandle.trim()}" to row ${i + 2}`
			);
		}
	}
	Logger.log(
		'Created mapping of Discord handles to rows in "Overall Scores" sheet.'
	);

	// Iterate over the month sheet and update the "Overall Scores" sheet
	const monthData = monthSheet
		.getRange(2, 1, monthSheet.getLastRow() - 1, 5)
		.getValues(); // Columns A to E
	...
	for (let i = 0; i < monthData.length; i++) {
		...
		Logger.log(
			`Processing row ${
				i + 2
			} in month sheet: Submitter "${submitterDiscordHandle}", Final Score: ${finalScore}`
		);
		...
		if (!overallRow) {
			Logger.log(
				`Submitter "${submitterDiscordHandle}" not found in "Overall Scores" sheet. Skipping.`
			);
			continue;
		}
		...
		overallScoreSheet
			.getRange(overallRow, monthColumnIndex)
			.setValue(finalScore);
		Logger.log(
			`Updated final score for "${submitterDiscordHandle}" in "Overall Scores" sheet at row ${overallRow}, column ${monthColumnIndex}.`
		);
	}
...
function sendEvaluationReminderEmails() {
	try {
		...
		const reviewData = reviewLogSheet
			.getRange(2, 1, lastRow - 1, 4)
			.getValues();
		const allEvaluators = reviewData
			.flatMap((row) => [row[1], row[2], row[3]])
			.filter((email) => email); // Вытаскиваем всех оценщиков

		// Get the list of already responded evaluators from Form Responses
		const formResponses = formResponseSheet
			.getRange(
				2,
				1,
				formResponseSheet.getLastRow() - 1,
		...
		const nonRespondents = allEvaluators.filter(
			(email) => !respondedEvaluators.includes(email)
		);
