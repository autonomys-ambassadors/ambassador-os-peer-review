...
function checkEvaluationWindowStart() {
	const startDateProperty = PropertiesService.getScriptProperties().getProperty(
	...
	if (!startDateProperty) {
		...
		SpreadsheetApp.getUi().alert(
			"Error: Evaluation window start date is not set."
		);
		return false;
	}
	...
	if (daysSinceStart < 7) {
		...
		Logger.log(
			"7 days have passed since the evaluation window started; no warning needed."
		);
...
function copyFinalScoresToOverallScore() {
	try {
		...
		Logger.log(
			`Current month date for copying scores: ${currentMonthDate.toISOString()}`
		);
		...
		const existingColumns = overallScoreSheet
			.getRange(1, 1, 1, overallScoreSheet.getLastColumn())
			.getValues()[0];
		const monthColumnIndex =
			existingColumns.findIndex(
				(header) =>
					header instanceof Date &&
					header.getTime() === currentMonthDate.getTime()
			) + 1;

		if (monthColumnIndex === 0) {
			Logger.log(
				`Column for "${monthSheetName}" not found in Overall score sheet.`
			);
			return;
		}
		...
		Logger.log(
			`Retrieved ${finalScores.length} scores from "${monthSheetName}" sheet.`
		);
		...
		finalScores.forEach(({ handle, score }) => {
			const rowIndex =
				overallHandles.findIndex((overallHandle) => overallHandle === handle) +
				2;
			if (rowIndex > 1 && score !== "") {
				overallScoreSheet.getRange(rowIndex, monthColumnIndex).setValue(score);
				Logger.log(
					`Copied score for handle ${handle} to row ${rowIndex} in Overall score sheet.`
				);
			}
		});
...
function checkAndCreateColumns() {
	const overallScoresSheet =
		SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
			OVERALL_SCORE_SHEET_NAME
		);
	const headersRange = overallScoresSheet
		.getRange(1, 1, 1, overallScoresSheet.getLastColumn())
		.getValues()[0];
	...
	if (penaltyPointsColIndex === 0) {
		...
		overallScoresSheet
			.getRange(1, penaltyPointsColIndex)
			.setValue("Penalty Points");
	...
	if (maxPenaltyPointsColIndex === 0) {
		...
		overallScoresSheet
			.getRange(1, maxPenaltyPointsColIndex)
			.setValue("Max 6-Month PP");
...
function detectNonRespondersPastMonths() {
	...
	if (hasRun === "true") {
		Logger.log(
			"Warning: This function has already been executed and is locked from repeated runs."
		);
	...
	const currentMonthName = Utilities.formatDate(
		currentMonthDate,
		spreadsheetTimeZone,
		"MMMM yyyy"
	);
	...
	for (let row = 2; row <= lastRow; row++) {
		let currentPenaltyPoints =
			sheetData[row - 1][penaltyPointsColIndex - 1] || 0;
	...
	overallScoresSheet
		...
		.setValues(
			sheetData.slice(1).map((row) => [row[penaltyPointsColIndex - 1]])
		);
	...
	Logger.log(
		"detectNonRespondersPastMonths completed. Function locked from repeated runs."
	);
}
...
function calculatePenaltyPointsForSubmissions() {
	...
	const formResponsesSheet = SpreadsheetApp.openById(
		AMBASSADOR_REGISTRY_SPREADSHEET_ID
	...
	const headersRange = overallScoresSheet
		...
		.getValues()[0];
	const penaltyPointsColIndex = headersRange.indexOf("Penalty Points") + 1;
	const reportingMonthDate = getPreviousMonthDate(
		SpreadsheetApp.getActiveSpreadsheet().getSpreadsheetTimeZone()
	...
	const submissionWindowStart = new Date(
		PropertiesService.getScriptProperties().getProperty("submissionWindowStart")
	);
	...
	registrySheet
		...
		.forEach((row, index) => {
			...
			if (!submittedEmails.has(ambassadorEmail)) {
				...
				const rowInScores = overallScoresSheet
					.createTextFinder(discordHandle)
					.findNext()
					?.getRow();

				if (rowInScores) {
					const currentPenaltyPoints =
						overallScoresSheet
							.getRange(rowInScores, penaltyPointsColIndex)
							.getValue() || 0;
					overallScoresSheet
						.getRange(rowInScores, penaltyPointsColIndex)
						.setValue(currentPenaltyPoints + 1);
					overallScoresSheet
						.getRange(rowInScores, monthColumnIndex)
						.setBackground(COLOR_MISSED_SUBMISSION)
						.setValue("");
					Logger.log(
						`Added 1 penalty point for missing submission for ${discordHandle}.`
					);
				}
			}
		});
...
function calculatePenaltyPointsForEvaluations() {
	try {
		...
		const evaluationResponsesSheet = SpreadsheetApp.openById(
			AMBASSADORS_SCORES_SPREADSHEET_ID
		).getSheetByName(EVAL_FORM_RESPONSES_SHEET_NAME);
		const overallScoresSpreadsheet = SpreadsheetApp.openById(
			AMBASSADORS_SCORES_SPREADSHEET_ID
		);
		const overallScoresSheet = overallScoresSpreadsheet.getSheetByName(
			OVERALL_SCORE_SHEET_NAME
		);
		...
		const spreadsheetTimeZone =
			overallScoresSpreadsheet.getSpreadsheetTimeZone();
		...
		const monthColumnIndex =
			headersRange.findIndex(
				(header) =>
					header instanceof Date &&
					header.getTime() === deliverableMonthDate.getTime()
			) + 1;
		...
		const sheetData = overallScoresSheet
			.getRange(
				2,
				1,
				overallScoresSheet.getLastRow() - 1,
				overallScoresSheet.getLastColumn()
			)
			.getValues();
			...
			evaluators.forEach((evaluator) => {
				...
				if (!validEvaluators.includes(evaluator)) {
					...
					if (evaluatorRow && !missedEvaluators.has(discordHandle)) {
						...
						let currentPenaltyPoints =
							sheetData[evaluatorRow - 2][penaltyPointsColIndex - 1] || 0;
						sheetData[evaluatorRow - 2][penaltyPointsColIndex - 1] =
							currentPenaltyPoints + 1;

						// Update background color to reflect the penalty
						const cellColor =
							backgroundData[evaluatorRow - 2][monthColumnIndex - 1];
						if (cellColor === COLOR_MISSED_SUBMISSION) {
							backgroundData[evaluatorRow - 2][monthColumnIndex - 1] =
								COLOR_MISSED_SUBM_AND_EVAL; // For both violations
						} else {
							backgroundData[evaluatorRow - 2][monthColumnIndex - 1] =
								COLOR_MISSED_EVALUATION; // For missed evaluation
						}
					}
				}
			});
		...
		Logger.log(
			`Error in calculatePenaltyPointsForEvaluations: ${error.message}`
		);
	}
}
...
function calculateMaxPenaltyPointsForSixMonths() {
	...
	if (penaltyPointsCol === 0 || maxPPCol === 0) {
		Logger.log(
			"Error: Either Penalty Points or Max 6-Month PP column not found."
		);
		return;
	}
	...
	for (let row = 2; row <= lastRow; row++) {
		...
		const backgroundColors = overallScoresSheet
			...
			.getBackgrounds()[0];
		Logger.log(
			`Row ${row}: Collected background colors for all month columns.`
		);
		...
		for (let i = 0; i <= monthColumns.length - 6; i++) {
			...
			Logger.log(
				`Checking 6-month period starting from column ${
					monthColumns[i]
				} (${Utilities.formatDate(
					headers[monthColumns[i] - 1],
					spreadsheetTimeZone,
					"MMMM yyyy"
				)})`
			);
			...
			for (let j = i; j < i + 6; j++) {
				...
				Logger.log(
					`Row ${row}, Column ${monthColumns[j]}: Detected background color = ${cellBackgroundColor}`
				);
				...
				switch (cellBackgroundColor) {
					case COLOR_OLD_MISSED_SUBMISSION: // for old months' didn't submit
						sixMonthTotal += 1;
						Logger.log(
							`Adding 1 point for COLOR_OLD_MISSED_SUBMISSION at column ${monthColumns[j]}`
						);
						break;
					case COLOR_MISSED_SUBMISSION: // didn't submit
						sixMonthTotal += 1;
						Logger.log(
							`Adding 1 point for COLOR_MISSED_SUBMISSION at column ${monthColumns[j]}`
						);
						break;
					case COLOR_MISSED_EVALUATION: // didn't evaluate
						sixMonthTotal += 1;
						Logger.log(
							`Adding 1 point for COLOR_MISSED_EVALUATION at column ${monthColumns[j]}`
						);
						break;
					case COLOR_MISSED_SUBM_AND_EVAL: // didn't submit and didn't evaluate
						sixMonthTotal += 2;
						Logger.log(
							`Adding 2 points for COLOR_MISSED_SUBM_AND_EVAL at column ${monthColumns[j]}`
						);
						break;
					default:
						Logger.log(
							`No penalty for background color at column ${monthColumns[j]}`
						);
				}
			}

			Logger.log(
				`Total penalty points for this 6-month period: ${sixMonthTotal}`
			);
		...
		if (maxPP >= 3) {
			...
			Logger.log(
				`Max PP >= 3. Setting red background for row ${row} in Max 6-Month PP column.`
			);
		}
...
function expelAmbassadors() {
	...
	const registryHeaders = registrySheet
		.getRange(1, 1, 1, registrySheet.getLastColumn())
		.getValues()[0];
	...
	const discordHandleColIndex =
		registryHeaders.indexOf(AMBASSADOR_DISCORD_HANDLE_COLUMN) + 1;
	...
	ambassadorData.forEach((row, i) => {
		...
		if (maxPenaltyPoints >= 3) {
			...
			if (registryRowIndex > 1) {
				...
				const currentEmail = registrySheet
					.getRange(registryRowIndex, emailColIndex)
					.getValue();
				const currentDiscordHandle = registrySheet
				...
				if (!currentEmail.startsWith("(EXPELLED) ")) {
					registrySheet
						.getRange(registryRowIndex, emailColIndex)
						.setValue(`(EXPELLED) ${currentEmail}`);
				}
...
function sendExpulsionNotifications(discordHandle) {
	Logger.log(
		`Sending expulsion notifications for ambassador with discord handle: ${discordHandle}`
	);
	...
	if (registryRowIndex) {
		let email = registrySheet
			.getRange(registryRowIndex, emailColIndex)
			.getValue();

		// Check if the email is in expelled format and modify accordingly
		const expelledEmail = email.startsWith("(EXPELLED) ")
			? email
			: `(EXPELLED) ${email}`;

		const subject = "Expulsion from the Program";
		const body = EXPULSION_EMAIL_TEMPLATE.replace(
			"{AmbassadorEmail}",
			expelledEmail
		);
		...
		Logger.log(
			`Notification sent to sponsor for expelled ambassador: ${expelledEmail}.`
		);
	} else {
		Logger.log(
			`Error: Ambassador with discord handle ${discordHandle} not found in the registry.`
		);
	}
}
