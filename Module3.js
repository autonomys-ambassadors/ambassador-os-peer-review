...
function detectNonRespondersPastMonths() {
  ...
  if (hasRun === 'true') {
    ...
    SpreadsheetApp.getUi().alert("Warning! Processing Past Months function is designed to run only once. To allow a re-run, set 'detectNonRespondersPastMonthsRan' to 'false' in the script properties.");
  ...
  overallScoresSheet.getRange(2, penaltyPointsColIndex, lastRow - 1, 1).setValues(
...
function calculatePenaltyPointsForSubmissions() {
  ...
  const formResponsesSheet = SpreadsheetApp.openById(AMBASSADOR_REGISTRY_SPREADSHEET_ID).getSheetByName(FORM_RESPONSES_SHEET_NAME);
  const overallScoresSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(OVERALL_SCORE_SHEET_NAME);
  ...
  const reportingMonthName = Utilities.formatDate(reportingMonthDate, SpreadsheetApp.getActiveSpreadsheet().getSpreadsheetTimeZone(), 'MMMM yyyy');
  const monthColumnIndex = headersRange.findIndex(header => header instanceof Date && Utilities.formatDate(header, SpreadsheetApp.getActiveSpreadsheet().getSpreadsheetTimeZone(), 'MMMM yyyy') === reportingMonthName) + 1;
  ...
  const submittedEmails = new Set(formResponsesSheet.getRange(2, 1, formResponsesSheet.getLastRow() - 1, 2).getValues().filter(row => {
    const timestamp = new Date(row[0]);
    return timestamp >= submissionWindowStart && timestamp <= submissionWindowEnd;
  }).map(row => row[1].trim().toLowerCase()));

  registrySheet.getRange(2, 1, registrySheet.getLastRow() - 1, 1).getValues().forEach((row, index) => {
    ...
    if (!submittedEmails.has(ambassadorEmail)) {
      ...
      if (rowInScores) {
        ...
        overallScoresSheet.getRange(rowInScores, monthColumnIndex).setBackground(COLOR_MISSED_SUBMISSION).setValue('');
...
function calculatePenaltyPointsForEvaluations() {
  try {
    ...
    const evaluationResponsesSheet = SpreadsheetApp.openById(AMBASSADORS_SCORES_SPREADSHEET_ID).getSheetByName(EVAL_FORM_RESPONSES_SHEET_NAME);
    ...
    const monthColumnIndex = headersRange.findIndex(header => header instanceof Date && header.getTime() === deliverableMonthDate.getTime()) + 1;
    ...
    const sheetData = overallScoresSheet.getRange(2, 1, overallScoresSheet.getLastRow() - 1, overallScoresSheet.getLastColumn()).getValues();
    const backgroundData = overallScoresSheet.getRange(2, 1, overallScoresSheet.getLastRow() - 1, overallScoresSheet.getLastColumn()).getBackgrounds();
    ...
    overallScoresSheet.getRange(2, 1, overallScoresSheet.getLastRow() - 1, overallScoresSheet.getLastColumn()).setValues(sheetData);
    overallScoresSheet.getRange(2, 1, overallScoresSheet.getLastRow() - 1, overallScoresSheet.getLastColumn()).setBackgrounds(backgroundData);
...
function calculateMaxPenaltyPointsForSixMonths() {
  ...
  for (let col = 1; col <= lastColumn; col++) {
    ...
    if (cellValue instanceof Date) {
      ...
      Logger.log(`Found month column at index ${col} with date: ${Utilities.formatDate(cellValue, spreadsheetTimeZone, 'MMMM yyyy')}`);
    }
  }
  ...
  for (let row = 2; row <= lastRow; row++) {
    ...
    const backgroundColors = overallScoresSheet.getRange(row, monthColumns[0], 1, monthColumns.length).getBackgrounds()[0];
    ...
    for (let i = 0; i <= monthColumns.length - 6; i++) {
      ...
      Logger.log(`Checking 6-month period starting from column ${monthColumns[i]} (${Utilities.formatDate(headers[monthColumns[i] - 1], spreadsheetTimeZone, 'MMMM yyyy')})`);
