...
function createMonthSheetAndOverallColumn() {
	try {
		...
		monthSheet
			.getRange(2, 2, monthSheet.getMaxRows() - 1, 3)
			.setBackground("#ebeee3");
		monthSheet
			.getRange(2, 5, monthSheet.getMaxRows() - 1, 3)
			.setBackground("#ffffff");
		monthSheet
			.getRange(2, 8, monthSheet.getMaxRows() - 1, 3)
			.setBackground("#ebeee3");
		...
		const existingColumns = overallScoreSheet
			...
			.getValues()[0];
		Logger.log(
			`Existing columns in "Overall score": ${existingColumns.join(", ")}`
		);
		...
		if (columnExists) {
			Logger.log(
				`Column for "${deliverableMonthName}" already exists in "Overall score". Skipping creation.`
			);
			return;
		}
		...
		// Устанавливаем стандартное форматирование (без фона)
		...
		Logger.log(
			`Column for "${deliverableMonthName}" successfully added to "Overall score".`
		);
	} catch (error) {
...
function generateReviewMatrix() {
	try {
		...
		const registrySheet =
			registrySpreadsheet.getSheetByName(REGISTRY_SHEET_NAME);
		...
		submissionWindowEnd.setMinutes(
			submissionWindowStart.getMinutes() + SUBMISSION_WINDOW_MINUTES
		);
		...
		const allAmbassadorsEmails = registrySheet
			...
			.flat();
		Logger.log(
			`All Ambassadors Emails: ${JSON.stringify(allAmbassadorsEmails)}`
		);
...
function sendExemptionEmails(allEvaluators, assignedEvaluators) {
	...
	const exemptedEvaluators = allEvaluators.filter(
		(evaluator) => !assignedEvaluatorsSet.has(evaluator)
	);
...
function sendEvaluationRequests() {
	try {
		...
		const reviewLogSheet = SpreadsheetApp.openById(
			AMBASSADOR_REGISTRY_SPREADSHEET_ID
		).getSheetByName(REVIEW_LOG_SHEET_NAME); // Correct ID
			...
			reviewersEmails.forEach((reviewerEmail) => {
				try {
					...
					const message = REQUEST_EVALUATION_EMAIL_TEMPLATE.replace(
						...
						.replace("{EVALUATION_WINDOW_MINUTES}", EVALUATION_WINDOW_MINUTES);
...
function processEvaluationResponse(e) {
	try {
		...
		const submitterRows = monthSheet
			.getRange(2, 1, monthSheet.getLastRow() - 1, 1)
			.getValues();
		let row = null;
		for (let i = 0; i < submitterRows.length; i++) {
			if (
				submitterRows[i][0] &&
				submitterRows[i][0].toLowerCase() ===
					submitterDiscordHandle.toLowerCase()
			) {
		...
		const validGrades = gradesRange.filter(
...
function sendEvaluationReminderEmails() {
	try {
		...
		if (!reviewLogSheet || !formResponseSheet) {
			Logger.log(
				"Review Log sheet or Form Responses sheet not found. Exiting sendEvaluationReminderEmails."
			);
			return;
		}
		...
		const validEvaluators = new Set(
			getValidEvaluationEmails(formResponseSheet)
		); // A Set of valid evaluator emails
		...
		for (const [submitterEmail, evaluators] of Object.entries(
			reviewAssignments
		)) {
			evaluators.forEach((evaluatorEmail) => {
				// Count assigned evaluations per evaluator only once
				const assignedEvaluations = reviewAssignments[submitterEmail].includes(
					evaluatorEmail
				)
					? 1
					: 0;
				...
				const completedEvaluations = validEvaluators.has(evaluatorEmail)
					? 1
					: 0;
				...
				if (completedEvaluations < assignedEvaluations) {
					...
					Logger.log(
						`Evaluator ${evaluatorEmail} has not completed all evaluations.`
					);
				}
			});
		}
		...
		if (nonRespondents.size > 0) {
			...
			Logger.log(
				"All evaluations are completed within the time window. No reminders to send."
			);
		}
	} catch (error) {
	...
	try {
		...
		nonRespondents.forEach((evaluatorEmail) => {
			...
			if (result) {
				...
				Logger.log(
					`Error: Could not find the ambassador with email ${evaluatorEmail}`
				);
			}
		});
	} catch (error) {
...
function setupEvaluationReminderTrigger(evaluationWindowStart) {
	try {
		...
		reminderTime.setMinutes(
			reminderTime.getMinutes() + EVALUATION_WINDOW_REMINDER_MINUTES
		);
		...
		PropertiesService.getScriptProperties().setProperty(
			"reminderTriggerSetupDate",
			setupDate
		);
		Logger.log(
			`Updated reminderTriggerSetupDate in Script Properties: ${setupDate}`
		);
	} catch (error) {
...
function setupEvaluationTriggers(evaluationWindowStart) {
	try {
		...
		PropertiesService.getScriptProperties().setProperty(
			...
			evaluationWindowStart.toISOString()
		);
		...
		evaluationWindowEnd.setMinutes(
			evaluationWindowEnd.getMinutes() + EVALUATION_WINDOW_MINUTES
		);
		Logger.log(
			`Evaluation window is from ${evaluationWindowStart} to ${evaluationWindowEnd}`
		);
