...
function requestEvaluationsModule() {
  ...
  setEvaluationWindowStart(evaluationStartTime);  // Save the evaluation start time for the evaluation window
  Logger.log(`Evaluation window start time set to: ${evaluationStartTime}`);
...
function createMonthSheetAndOverallColumn() {
  try {
    ...
    if (lastRow > 1) { // Ensuring there are raws for sorting
...
function sendEvaluationRequests() {
  try {
    ...
    const reviewLogSheet = SpreadsheetApp.openById(AMBASSADOR_REGISTRY_SPREADSHEET_ID).getSheetByName(REVIEW_LOG_SHEET_NAME);  // Correct ID
  ...
  try {
    ...
    for (let col = 2; col <= 8; col += 3) { 
      ...
      if (cellValue === evaluatorDiscordHandle) {
        ...
        Logger.log(`Updated grade and remarks for submitter ${submitterDiscordHandle} by evaluator ${evaluatorDiscordHandle}. Grade: ${grade}, Remarks: ${remarks}`);
        gradeUpdated = true;
        break;
      }
    }
...
function sendEvaluationReminderEmails() {
  try {
    ...
    const formResponseSheet = SpreadsheetApp.openById(AMBASSADORS_SCORES_SPREADSHEET_ID).getSheetByName(EVAL_FORM_RESPONSES_SHEET_NAME);
...
function setupEvaluationReminderTrigger(evaluationStartTime) {
  try {
    ...
    // Calculate reminder time by adding EVALUATION_WINDOW_REMINDER_MINUTES to evaluationStartTime
...
function setupEvaluationTriggers(evaluationStartTime) {
  try {
    ...
    const evaluationWindowEnd = new Date(evaluationStartTime);
    evaluationWindowEnd.setMinutes(evaluationWindowEnd.getMinutes() + EVALUATION_WINDOW_MINUTES);
    Logger.log(`Evaluation window is from ${evaluationStartTime} to ${evaluationWindowEnd}`);

    // Set up the evaluation reminder trigger
    setupEvaluationReminderTrigger(evaluationStartTime);
  } catch (error) {
