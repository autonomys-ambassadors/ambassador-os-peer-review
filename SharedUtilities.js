...
{SubmissionFormURL}
The deadline is {SUBMISSION_WINDOW_MINUTES} days.
Thank you,
Program Team
`;
...
{EvaluationFormURL}
The deadline is {EVALUATION_WINDOW_MINUTES} days.
Best regards,
Program Team
`;
...
function refreshScriptState() {
	...
	Logger.log(
		"Script state refreshed: cache cleared, variables refreshed, and flush completed."
	);
}
function clearCache() {
...
function refreshGlobalVariables() {
	...
	AMBASSADOR_REGISTRY_SPREADSHEET_ID =
		"1AIMD61YKfk-JyP6Aia3UWke9pW15bPYTvWc1C46ofkU";
	AMBASSADORS_SCORES_SPREADSHEET_ID =
		"1RJzCo1FgGWkx0UCYhaY3SIOR0sl7uD6vCUFw55BX0iQ";
	...
	Logger.log(
		"Templates and constants reinitialized to ensure accurate processing."
	);
	...
	Logger.log(
		"Color for missed submiss and eval: " + COLOR_MISSED_SUBM_AND_EVAL
	);
...
function sendEmailNotification(recipientEmail, subject, body) {
	try {
		if (!testing) {
			...
			Logger.log(
				`Test mode: Email to be sent to: ${recipientEmail}, Subject: ${subject}`
			);
		}
	} catch (error) {
...
function getEvaluationWindowStart() {
	const timeStr = PropertiesService.getScriptProperties().getProperty(
		"evaluationWindowStart"
	);
...
function setValidSubmissionResponses(emails) {
	PropertiesService.getScriptProperties().setProperty(
		"validSubmissionResponses",
		JSON.stringify(emails)
	);
}
function getValidSubmissionResponses() {
	const emailsStr = PropertiesService.getScriptProperties().getProperty(
		"validSubmissionResponses"
	);
...
function getValidEvaluationEmails(evaluationResponsesSheet) {
	...
	const evaluationWindowEnd = new Date(
		evaluationWindowStart.getTime() + EVALUATION_WINDOW_MINUTES * 60 * 1000
	);

	Logger.log(
		`Evaluation window: ${evaluationWindowStart} - ${evaluationWindowEnd}`
	);
	...
	const validEvaluators = evaluationResponsesSheet
		...
		.filter((row) => {
			...
			const isWithinWindow =
				responseTimestamp >= evaluationWindowStart &&
				responseTimestamp <= evaluationWindowEnd;

			if (isWithinWindow) {
				Logger.log(
					`Valid evaluator found: ${email} (Response time: ${responseTimestamp})`
				);
				return true;
			}
			return false;
	...
	Logger.log(
		`Valid evaluators (within time window): ${validEvaluators.join(", ")}`
	);
...
function findRowByDiscordHandle(discordHandle) {
	const overallScoresSheet = SpreadsheetApp.openById(
		AMBASSADORS_SCORES_SPREADSHEET_ID
	).getSheetByName(OVERALL_SCORE_SHEET_NAME);
	...
	const rowIndex = handlesColumn.findIndex(
		(handle) => handle === discordHandle
	);
