...
function requestSubmissionsModule() {
	...
	emails.forEach((email, index) => {
		...
		const message = REQUEST_SUBMISSION_EMAIL_TEMPLATE.replace(
			...
			.replace("{SUBMISSION_WINDOW_MINUTES}", SUBMISSION_WINDOW_MINUTES);
	...
	// Set a trigger to check for non-respondents and send reminders after 6 days (minutes for testing)
...
function setupSubmissionReminderTrigger(submissionStartTime) {
	...
	triggerDate.setMinutes(
		triggerDate.getMinutes() + SUBMISSION_WINDOW_REMINDER_MINUTES
	); // Setup reminder trigger
